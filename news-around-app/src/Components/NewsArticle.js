...
function NewsArticle({ allArticles, categoryArticles, getCategoryHeadlines, getCountryHeadlines }) {
  ...
  return (
    <SimpleGrid columns={columnCount} spacing={10} justifyItems="center" marginLeft="100px" marginRight="100px">
      {categoryArticles.map(({ title, author, source, url, urlToImage }, index) => (
      ...
      {getCategoryHeadlines && allArticles.map(({ title, author, source, url, urlToImage }, index) => (
        <Link href={url} isExternal key={title}>
          <Card w="500px" h="200px">
            <CardBody>
              <Text color="black" fontWeight="semibold" marginBottom={3}>
